...
@section('pagetitle', 'Custom Fields Data List')
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Custom Fields Data</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          @include('backend.layout.messages')
            <div class="card">
              <div class="card-header tblcard">
                <h4 class="card-title">{{ ucwords($customFields->name) ?? ''}} ({{'Field Type '.$customFields->input_field_type}})</h4>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped mt-3">
                  ...
                  <tbody>
                    @if($customFieldsdata)
                    @foreach($customFieldsdata as $key => $customFielddata)
                  <tr>
                    <td>{{ $key+1 }}</td>
                    <td>{{ucwords($customFielddata->name) ?? ''}}</td>
                    <td class="project-state"> @if($customFielddata->status == '1') <span class="badge badge-success">Active</span>@elseif($customFielddata->status == '2') <span class="badge badge-danger">Deactive</span>@endif</td>
                    <td class="project-actions">
                      @if(in_array('customFieldsData.edit', $groupsWithRoles))
                      <a class="btn btn-info btn-sm" href="{{ route('customFieldsData.edit', ['id'=>$customFields->id,'tdid'=>$customFielddata->id]) }}">
                          <i class="fas fa-pencil-alt"></i>  Edit
                      </a>
                      @endif
                      @if(in_array('customFieldsData.destroy', $groupsWithRoles))
                      <form method="POST" action="{{ route('customFieldsData.destroy', $customFielddata->id) }}" style="display: inline-block;">
                        @csrf
...
<script type="text/javascript">
     ...
     $('.show_confirm').click(function(event) {
          ...
          swal({
              title: `Are you sure you want to delete this custom field data?`,
