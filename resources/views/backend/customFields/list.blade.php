    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3>Custom Fields 
               @if(in_array('custom-fields.create', $groupsWithRoles))<a href="{{ route('custom-fields.create') }}"><button class="btn btn-primary">Add Custom Field</button></a>@endif
              <a href="{{ route('custom-fields.index') }}"><button class="btn btn-primary">Back</button></a></h3>
          </div>
          <div class="col-sm-6">
            ...
            <div class="card">
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped mt-3">
                  ...
                  <tr>
                    <td>{{ $key+1 }}</td>
                    <td>{{ucwords($customField->name) ?? ''}} @if($customField->input_required != "") <span class="text-danger">*</span> @endif</td>
                    <td>{{$customField->input_field_type ?? ''}} </td>
                    <td>{{$customField->order_no ?? ''}} </td>
                    <td class="project-state"> @if($customField->status == '1') <span class="badge badge-success">Active</span>@elseif($customField->status == '2') <span class="badge badge-danger">Deactive</span>@endif</td>
                    <td class="project-actions">
                      @if(in_array('custom-fields.show', $groupsWithRoles))
                          @if($customField->input_field_type == 'Select')
                          <a class="btn btn-primary btn-sm" href="{{ route('custom-fields.show', $customField->id) }}">
                            {!! App\Models\CustomFields::checkCFDexits($customField->id) ?? '<i class="fas fa-plus mx-2"></i> Add Data' !!}
                          </a>
                          @endif
                      @endif
                      @if(in_array('custom-fields.edit', $groupsWithRoles))
                          <a class="btn btn-info btn-sm" href="{{ route('custom-fields.edit', $customField->id) }}">
                              <i class="fas fa-pencil-alt"></i>  Edit
                          </a>
                      @endif
                      @if(in_array('custom-fields.destroy', $groupsWithRoles))
                          <form method="POST" action="{{ route('custom-fields.destroy', $customField->id) }}" style="display: inline-block;">
                            @csrf
