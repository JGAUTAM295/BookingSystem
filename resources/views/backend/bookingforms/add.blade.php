...
@section('pagetitle', 'Booking Form Add')
...
@section('content')
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h3>{{$form->title ?? ''}} <a href="{{ route('booking-forms.index') }}"><button class="btn btn-primary">Back</button></a></h3>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ url('/') }}">Home</a></li>
              <li class="breadcrumb-item active">Booking Form Add</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
      ...
      <div class="row">
        ...
        @endif
          <div class="card card-primary">
            ...
            <div class="card-body">
                <form action="{{ route('booking-forms.store') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                      ...
                      @if($cfs)
                        ...
                        <div class="form-group col">
                          ...
                          <div class="input-group date" id="reservationdatetime" data-target-input="nearest">
                            <input type="text" id="input{{ucwords($cf->slug) ?? ''}}" class="form-control datetimepicker-input" data-target="#reservationdatetime" name="{{$cf->slug.'='.$cf->id ?? ''}}" @if($cf->input_required != "") required @endif/>
                          ...
                          <input type="email" id="input{{ucwords($cf->slug) ?? ''}}" class="form-control {{strtolower($cf->input_field_type).'css'}}" name="{{$cf->slug.'='.$cf->id ?? ''}}" @if($cf->input_required != "") required @endif>
